...
def get_phoenix_model(feh, alpha, teff, logg, workingfolder):
    ...
    stellarmodels_folder_path = workingfolder + 'data/stellarmodels/'
    phoenix_folder_path = workingfolder + 'data/stellarmodels/phoenix/'
    model_folder_path = workingfolder + 'data/stellarmodels/phoenix/' + phoenix_met_and_alpha + '/'
...
def get_vega():
    ...
    """
    data = fits.getdata('data/alpha_lyr_stis_009.fits', header=False)
...
def scale_spectrum(w, f, jmag):
    ...
    wT, TT = np.loadtxt('data/jband_transmission.dat', unpack=True, usecols=(0, 1))
